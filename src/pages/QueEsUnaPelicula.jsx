  ...
  return (
    <div className="border border-ink-200 rounded-2xl overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between p-5 text-left bg-white hover:bg-ink-50 transition-colors"
      >
        <span className="font-semibold text-ink-900 pr-4 text-sm sm:text-base">{faq.q}</span>
        {open ? (
          <FiChevronUp className="text-brand-blue flex-shrink-0" size={20} />
        ) : (
          <FiChevronDown className="text-ink-400 flex-shrink-0" size={20} />
        )}
      </button>
      {open && (
        <div className="px-5 pb-5 bg-brand-light border-t border-ink-100">
          <p className="text-ink-600 text-sm leading-relaxed pt-4">{faq.a}</p>
        </div>
      )}
    </div>
  )
          ...
          />
            ...
            <div>
              ...
              <div className="space-y-4 text-ink-600 leading-relaxed">
                <p>
                  Las películas para cristales son <strong className="text-ink-900">laminados delgados</strong> que
                  se instalan sobre la superficie del vidrio con el objetivo de mejorar su funcionalidad y rendimiento.
                </p>
                <p>
                  Están fabricadas con capas de <strong className="text-ink-900">poliéster tratado, metales reflectivos
                  o nanopartículas cerámicas</strong>, dependiendo del tipo de película.
                </p>
                <p>
                  Se instalan por la <strong className="text-ink-900">cara interior del cristal</strong> y no dañan
                  el vidrio original.
                </p>
              </div>
